class BTStreamTest < Minitest::Test
  ...
  def test_trace_class
    ...
    comp_initialize_method = lambda { |self_component, configuration, params, data|
      ...
      stream = BT2::BTStream.new(stream_class: stream_class, trace: trace, id: 25)
      assert_instance_of(BT2::BTStream, stream)
      assert_equal(stream_class, stream.get_class)
      assert_equal(trace, stream.trace)
      assert_equal(25, stream.id)
      assert_nil(stream.name)
      stream.name = "name"
      assert_equal("name", stream.name)
      assert_equal({}, stream.user_attributes.value)
      stream.user_attributes = { "foo" => 15 }
      assert_equal({ "foo" => 15 }, stream.user_attributes.value)
    }
