module Babeltrace2
  ...
  class BTFieldClass::Blob < BTFieldClass
    ...
    def to_h
      res = super
      res[:media_type] = media_type if media_type
      res
    end

    def from_h(h)
      super
      self.media_type = h[:media_type] if h[:media_type]
      self
    end
  end
  ...
  class BTFieldClass::Blob::Dynamic < BTFieldClass::Blob
    ...
    def self.from_h(trace_class, h, stream_class_h = nil)
      length_field_location =
        BTFieldLocation.from_h(trace_class, h[:length_field_location]) if h[:length_field_location]
      o = self.new(trace_class: trace_class,
        length_field_location: length_field_location).from_h(h)
      h[:bt_field_class] = o
      o
    end
  end
